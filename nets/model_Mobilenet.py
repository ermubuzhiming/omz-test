...
from keras.layers import merge,Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, SeparableConv2D, \
                        Reshape, Lambda
...
def re_shape(y, num_anchors, num_classes):
    """

    :param y:
    :return: reshaped featuremap
    """
    num_anchors = num_anchors // 3
    grid_size = K.int_shape(y)
    dim = grid_size[1] * grid_size[2]
    bbox_attrs = 5 + num_classes
    y = Reshape((-1, num_anchors * dim, bbox_attrs))(y)
    print(K.int_shape(y))
    return y
...
def yolo_body(inputs, num_classes):
    ...
    num_anchors = 3
    # inputs1 = Lambda(lambda x: x / 255.)(inputs)
    mobilenetv2 = MobileNetV2(input_tensor=inputs, weights=None)

    f1 = mobilenetv2.get_layer('block_16_project_BN').output
    # f1 :13 x 13 x 1024
    x, y1 = make_last_layers(f1, 512, num_anchors * (num_classes + 5))
    x = compose(
            DarknetConv2D_BN_Leaky(256, (1,1)),
    ...
    x = Concatenate()([x, f3])
    x, y3 = make_last_layers(x, 128, num_anchors*(num_classes+5))
    #y1 = re_shape(y1, num_anchors, num_classes)
    #y2 = re_shape(y2, num_anchors, num_classes)
    #y3 = re_shape(y3, num_anchors, num_classes)
    return Model(inputs=inputs, outputs=[y1, y2, y3])
